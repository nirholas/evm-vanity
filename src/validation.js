...
import { eip55Checksum as checksumAddress } from './address.js';
...
/**
 * EIP-55 checksum for a 40-character lowercase hex body (no `0x`).
 *
 * The canonical implementation lives in [address.js](./address.js), which works
 * on full `0x…` addresses; this wrapper keeps the body-only signature the
 * grinder UI was written against so there is still exactly one algorithm.
 *
 * @param {string} lowerHex 40 lowercase hex characters
 * @returns {string} 40 checksummed hex characters
 */
export function eip55Checksum(lowerHex) {
	return checksumAddress(`0x${lowerHex}`).slice(2);
}
